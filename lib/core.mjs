...
// pi 的 input 数组 vs 由官网能力推导的模态（text 恒有；vision→image；pdf→pdf）
function compareAuditInput(field, config, entry) {
  const vision = dig(entry, ["vision"]);
  const pdf = dig(entry, ["pdf"]);
  const cfg = Array.isArray(config) ? config : [];
  const official = ["text"];
  if (vision === true) official.push("image");
  if (pdf === true) official.push("pdf");
  ...
  for (const item of extras) {
    ...
    else if (item === "pdf" && !isDeclared(pdf)) unknownExtras.push(item);
