...
export function matchModel(catalog, input, opts) {
  const strict = isDict(opts) && opts.strict === true;
  ...
  // 归一化阶段（去前缀/后缀/分隔符）
  const stages = normalizeStages(raw, prefixes).filter((s) => s.ops.length);

  // 严格模式（配置识别路径 audit / fix / protocols 使用）：只认 id / alias / legacy（大小写不敏感），
  // 归一化与模糊一律不自动命中，只作为「最接近」建议 —— 避免把中转变体误当成官网型号套用规格。
  if (strict) {
    for (const stage of stages) {
      const hit = exactLookup(stage.cand, keys);
      if (hit) {
        res.suggestion = hit[0] ? (hit[0].id ?? null) : null;
        res.suggestionKind = "normalized";
        res.ops = stage.ops.slice();
        return res;
      }
      const sepHit = sepMap.get(sepKey(stage.cand));
      if (sepHit !== undefined) {
        res.suggestion = sepHit ? (sepHit.id ?? null) : null;
        res.suggestionKind = "normalized";
        res.ops = stage.ops.slice().concat(["分隔符等价匹配 (-/./_ 视为相同)"]);
        return res;
      }
    }
    const rawSep = sepMap.get(sepKey(raw));
    if (rawSep !== undefined) {
      res.suggestion = rawSep ? (rawSep.id ?? null) : null;
      res.suggestionKind = "normalized";
      res.ops = ["分隔符等价匹配 (-/./_ 视为相同)"];
      return res;
    }
    // 归一化也无候选 → 落入下面的模糊 / 前缀建议
  } else {
...
function auditOneModel(models, entry, format) {
  // 配置识别由 id 触发：只认 id / alias / legacy（strict）；归一化与模糊只给建议，绝不自动套用官网规格。
  const match = matchModel(models, entry.input, { strict: true });
  const result = {
    input: entry.input,
    ...
    suggestionKind: match.entry ? null : (match.suggestionKind || null),
    diffs: [],
    clean: false,
  };
...
export function renderAudit(report, file) {
  ...
  for (const entry of report.entries) {
    if (!entry.matchedId) {
      ...
      if (entry.suggestion) {
        const hint = entry.suggestionKind === "normalized" ? "（归一化写法，未自动采用官网规格）" : "";
        lines.push(`    最接近: ${entry.suggestion}${hint}`);
      }
      lines.push("");
      continue;
    }
...
// fill 模式：未声明的字段也生成补齐项（from: null + missing 标记），官方未文档化仍 skip。
function planNumChange(entry, field, path, raw, official, opts) {
  const fill = !!(opts && opts.fill);
  if (!declaredAt(raw, path)) {
    if (!fill) return;
    ...
    entry.changes.push({ field, path: path.slice(), from: null, to: official, missing: true });
    return;
  }
...
// fill 模式：未声明的字段也生成补齐项（from: null + missing 标记），官方未文档化仍 skip。
function planBoolChange(entry, field, path, raw, official, opts) {
  const fill = !!(opts && opts.fill);
  if (!declaredAt(raw, path)) {
    if (!fill) return;
    if (!isDeclared(official)) {
...
// fill 模式：未声明时补齐为官网全部档位。
function planOpencodeVariants(entry, raw, e, opts) {
  const fill = !!(opts && opts.fill);
  if (!declaredAt(raw, ["variants"])) {
    if (!fill) return;
    const effortsF = valOrNone(e, ["reasoning", "effort_values"]);
    if (!Array.isArray(effortsF) || !effortsF.length) {
      entry.skipped.push({ field: "efforts", reason: "官网未文档化" });
      return;
    }
    const built = {};
    for (const level of effortsF) built[level] = { reasoningEffort: level };
    entry.changes.push({
      field: "efforts",
      path: ["variants"],
      from: null,
...
// name 补齐：仅 fill 模式；仅当 name 已声明、是字符串且 trim() 后为空时，
// 补为配置中的模型 id（与 `apifix <id> --emit opencode` 默认 name 行为一致）。
// 未声明 name 不动（不新增字段）；name 非空时保留用户自定义展示名。
function planNameChange(entry, raw, path, opts) {
  if (!(opts && opts.fill)) return;
...
function planOpencodeModel(entry, raw, e, opts) {
  planNameChange(entry, raw, ["name"], opts);
  planNumChange(entry, "context", ["limit", "context"], raw, valOrNone(e, ["context_window"]), opts);
  planNumChange(entry, "output", ["limit", "output"], raw, valOrNone(e, ["max_output_tokens"]), opts);
  planBoolChange(entry, "reasoning", ["reasoning"], raw, valOrNone(e, ["reasoning", "supported"]), opts);
  planBoolChange(entry, "temperature", ["temperature"], raw, valOrNone(e, ["sampling", "temperature", "supported"]), opts);
  planBoolChange(entry, "tool_call", ["tool_call"], raw, valOrNone(e, ["tools", "function_calling"]), opts);
  planBoolChange(entry, "attachment", ["attachment"], raw, valOrNone(e, ["vision"]), opts);
  planOpencodeVariants(entry, raw, e, opts);
}

// pi thinkingLevelMap：与 emitPi 相同的 7 键推导逻辑，整体替换；
// 原 map 含 7 键之外的键 → 记 skipped（仍会替换掉）。
// fill 模式：未声明时整体补齐为官网推导 map。
function planPiLevelMap(entry, raw, e, opts) {
  const fill = !!(opts && opts.fill);
  if (!declaredAt(raw, ["thinkingLevelMap"])) {
    if (!fill) return;
    const supportedF = valOrNone(e, ["reasoning", "supported"]);
    if (!isDeclared(supportedF)) {
      entry.skipped.push({ field: "efforts", reason: "官网未文档化" });
      return;
    }
    entry.changes.push({
      ...
      from: null,
      to: cloneValue(piThinkingLevelMap(e)),
      missing: true,
    });
    return;
  }
...
// fill 模式：未声明时补齐为官网推导数组。
function planPiInput(entry, raw, e, opts) {
  const fill = !!(opts && opts.fill);
  if (!declaredAt(raw, ["input"])) {
    if (!fill) return;
    const visionF = valOrNone(e, ["vision"]);
    if (!isDeclared(visionF)) {
    ...
    entry.changes.push({
      ...
      path: ["input"],
      from: null,
      to: visionF === true ? ["text", "image"] : ["text"],
      missing: true,
    });
    return;
  }
...
function planPiModel(entry, raw, e, opts) {
  planNameChange(entry, raw, ["name"], opts);
  planNumChange(entry, "context", ["contextWindow"], raw, valOrNone(e, ["context_window"]), opts);
  planNumChange(entry, "output", ["maxTokens"], raw, valOrNone(e, ["max_output_tokens"]), opts);
  planBoolChange(entry, "reasoning", ["reasoning"], raw, valOrNone(e, ["reasoning", "supported"]), opts);
  planPiLevelMap(entry, raw, e, opts);
  planPiInput(entry, raw, e, opts);
}
...
export function planConfigFixes(catalog, config, format, opts) {
  const fill = !!(opts && opts.fill);
  const models = asModelList(catalog);
  ...
  if (format === "opencode") {
    ...
    for (const providerName of Object.keys(providers).sort(cmpStr)) {
      ...
      for (const modelId of Object.keys(modelMap).sort(cmpStr)) {
        ...
        const match = matchModel(models, modelId, { strict: true });
        const entry = {
          input: modelId,
        ...
        if (match.entry) planOpencodeModel(entry, raw, match.entry, { fill });
        all.push(entry);
      }
    }
  } else if (format === "pi") {
    ...
    for (const providerName of Object.keys(providers).sort(cmpStr)) {
      ...
      for (let index = 0; index < list.length; index++) {
        ...
        if (match.entry) planPiModel(entry, raw, match.entry, { fill });
        all.push(entry);
      }
    }
  } else if (format === "claude") {
    ...
    for (const key of CLAUDE_MODEL_KEYS) {
      ...
      if (typeof value !== "string" || !value) continue;
      const match = matchModel(models, value, { strict: true });
      const entry = {
        input: value,
        provider: key,
        kind: match.entry ? match.kind : "none",
    ...
    for (const item of extracted) {
      const match = matchModel(models, item.input, { strict: true });
      all.push({
        input: item.input,
  ...
  return { format, entries, summary, fill };
}
...
// fill 模式下，缺失标记（from === null 且 missing: true）显示为「（缺失）」。
function fixValueText(value, missing) {
...
// 文本修复计划：按模型分组（风格参照 renderAudit）——
// 有可修项 / 需人工确认 / 未收录的条目都会逐条列出，谁会被改、谁不会被改一目了然。
export function renderFixPlan(plan, file) {
  const label = file === "-" ? "<stdin>" : String(file);
  const lines = [`修复 ${label}（识别为 ${plan.format} 配置）`, "═".repeat(40)];

  const shown = plan.entries.filter((entry) =>
    !entry.matchedId || entry.changes.length || entry.skipped.length);
  ...
  for (const entry of shown) {
    // 未收录：明确点名 + 给最接近候选（这类条目不参与修正，也不会套用官网规格）
    if (!entry.matchedId) {
      lines.push(`[?] ${entry.input} → ${AUDIT_NOT_FOUND_TEXT}`);
      if (entry.suggestion) {
        const hint = entry.suggestionKind === "normalized" ? "（归一化写法，未自动采用官网规格）" : "";
    ...
    const providerText = entry.duplicate && entry.provider ? `（${entry.provider}）` : "";
    const arrow = entry.input === entry.matchedId ? "" : ` → ${entry.matchedId}`;
    const mark = entry.changes.length ? `（待修 ${entry.changes.length} 项）` : "";
    lines.push(`${kindLabel} ${entry.input}${providerText}${arrow}${mark}`);

    let fromWidth = 0;
    let toWidth = 0;
    for (const change of entry.changes) {
      fromWidth = Math.max(fromWidth, displayWidth(fixValueText(change.from, change.missing)));
      toWidth = Math.max(toWidth, displayWidth(fixValueText(change.to)));
    }
    for (const change of entry.changes) {
      const name = AUDIT_FIELD_LABELS[change.field] || change.field;
      const fromText = pad(fixValueText(change.from, change.missing), fromWidth, "right");
  ...
  if (s.unmatched) {
    lines.push("提示：未收录条目不参与修正——确认型号后改用官网 id，或在 catalog 里登记 alias 锚定。");
  }
...
const OPENCODE_NPM_PROTOCOLS = {
  ...
  // @ai-sdk/openai = OpenAI 兼容接口格式（本项目协议词表里的 "openai"）：npm 字段本身就是
  // 格式声明，不标"推断"；比对时按 OpenAI 家族（chat_completions / responses）处理。
  "@ai-sdk/openai": { protocol: "openai", inferred: false },
};

// baseURL 细化：路径含 /responses 时按 responses 推断（URL 启发式，故 inferred=true）。
function inferOpencodeOpenai(baseUrl) {
  ...
  return { ...OPENCODE_NPM_PROTOCOLS["@ai-sdk/openai"] };
}
...
export function planCodexFixes(catalog, text) {
  ...
  const match = input
    ? matchModel(models, input, { strict: true })
    : { entry: null, kind: "none", matchedId: null, suggestion: null, suggestionKind: null };
...
// @ai-sdk/openai 同时可走 chat_completions 与 responses（baseURL 未指明 /responses 时不细分）。
// 比对时按「OpenAI 家族」处理：模型支持其中任一即判匹配（渲染层的协议列仍标注 (推断)）。
const OPENAI_FAMILY = ["chat_completions", "responses"];
...
function nativeProtocolCheck(models, modelId, protocol, inferred) {
  // 配置识别由 id 触发：只认 id / alias / legacy（strict），归一化写法不算命中
  const match = matchModel(models, modelId, { strict: true });
  ...
  const wanted = protocol === "openai" ? OPENAI_FAMILY : [protocol];
  if (wanted.some((p) => supported.includes(p))) {
...
export function protocolsReport(catalog, sources) {
  ...
  for (const source of sources || []) {
    ...
    const store = source.store === true || strategy === "pi-store";
    let extracted = [];
    ...
    for (const item of extracted) {
      ...
      providers.push({
        ...
        file: file ?? null,
        // 模型库（pi models-store.json 等自动生成的模型清单）与手写配置分流：
        // 模型库里的「未收录」是厂商目录里的型号，不代表配置有问题，渲染时折叠成一行、不计入配置统计。
        store: store === true,
        models: modelRows,
      });
    }
  ...
  const storeProviders = providers.filter((p) => p.store);
  const configProviders = providers.filter((p) => !p.store);
  const summary = {
    ...
    // unmapped 只统计手写配置（模型库的未收录单列）
    unmapped: configProviders.reduce((sum, p) => sum + p.models.filter((m) => m.status === "unmapped").length, 0),
    store_models: storeProviders.reduce((sum, p) => sum + p.models.length, 0),
    store_unmapped: storeProviders.reduce((sum, p) => sum + p.models.filter((m) => m.status === "unmapped").length, 0),
  };
...
export function renderProtocols(report) {
  ...
  for (const provider of report.providers) {
    ...
    if (provider.store) {
      ...
      const unmapped = provider.models.filter((m) => m.status === "unmapped").length;
      const mismatched = provider.models.filter((m) => m.status === "mismatch").length;
      const parts = [`${total - unmapped} 已锚定`, `${unmapped} 未收录`];
      if (mismatched) parts.push(`${mismatched} 协议不一致`);
      rows.push([provider.tool, provider.provider, protocolText, `模型库 ${total} 条`, parts.join(" · ") + "（不计入配置）"]);
      continue;
    }
  ...
  if (report.summary.store_models > 0) {
    lines.push(`模型库（自动生成，非手写配置）：${report.summary.store_models} 条，其中 ${report.summary.store_unmapped} 条目录未收录——不计入配置问题`);
  }
