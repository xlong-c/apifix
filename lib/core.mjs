...
function normalizeStages(rawInput) {
  const stages = [];
  ...
  if (cand.includes("/")) {
    ...
    if (VENDOR_PREFIXES.includes(prefix.toLowerCase()) && rest) {
      cand = rest;
...
export function matchModel(catalog, input) {
  ...
  for (const stage of normalizeStages(raw)) {
...
function buildClaudeEnv(entry, key) {
  const reasoning = asDict(dig(entry, ["reasoning"]));
  const lines = [`ANTHROPIC_MODEL=${key}`];

  const tb = "thinking_budget" in reasoning ? reasoning.thinking_budget : MISSING;
  if (isDict(tb) && tb.min !== null && tb.min !== undefined) {
    lines.push(`MAX_THINKING_TOKENS=${tb.min}`);
  } else {
...
export function emitExtra(entry, target, opts) {
  ...
  switch (target) {
    case "codex":
      return buildCodex(entry, key);
    case "claude-env":
      return buildClaudeEnv(entry, key);
    case "curl":
...
export function auditConfig(catalog, config, format) {
  ...
  const extracted = extractConfigEntries(stripCredentials(config), format);
...
export function renderAudit(report, file) {
  ...
  for (const entry of report.entries) {
    ...
    if (entry.diffs.some((d) => d.status === "diff")) {
      const emit = report.format === "pi" ? "pi" : "opencode";
...
export function applyConfigFixes(config, plan) {
  ...
  for (const entry of (plan && plan.entries) || []) {
    ...
    if (format === "opencode") {
      ...
      model = isDict(provider) && isDict(provider.models) ? provider.models[entry.ref.key] : null;
    } else if (format === "pi") {
      const provider = isDict(next) && isDict(next.providers) ? next.providers[entry.ref.provider] : null;
      model = isDict(provider) && Array.isArray(provider.models) ? provider.models[entry.ref.index] : null;
    }
...
// codex config.toml：窄行扫描（零依赖），只取 model / model_provider / wire_api / base_url。
export function parseCodexToml(text) {
  const result = { model: null, model_provider: null, providers: {} };
  let section = null;
  for (const line of String(text == null ? "" : text).split(/\r?\n/)) {
    ...
    if (section === null) {
      if (key === "model" || key === "model_provider") result[key] = value;
      continue;
    }
